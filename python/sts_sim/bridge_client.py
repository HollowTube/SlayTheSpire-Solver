...
def send_request(
    ...
    try:
        with socket.create_connection(
            (BRIDGE_HOST, BRIDGE_PORT), timeout=effective_timeout
        ) as sock:
            ...
            while True:
                ...
                if len(data) > MAX_RESPONSE_SIZE:
                    return {
                        "error": f"Bridge response exceeded {MAX_RESPONSE_SIZE} bytes"
                    }
...
def play_card(card_index: int, target_index: int = -1) -> dict:
    return send_request(
        "play_card", {"card_index": card_index, "target_index": target_index}
    )
...
def start_run_with_options(
    ...
    return start_run(
        character=character, ascension=ascension, seed=seed, fixture=fixture
    )
...
def act_and_wait(action: str, settle_timeout: float = 5.0, **params: Any) -> dict:
    ...
    while time.monotonic() < deadline:
        ...
        if current == prev_screen and any(
            s in current.upper() for s in _STABLE_SCREENS
        ):
            stable_count += 1
            if stable_count >= 1:
                break
...
def use_potion(potion_index: int, target_index: int = -1) -> dict:
    return send_request(
        "use_potion", {"potion_index": potion_index, "target_index": target_index}
    )
...
def shop_action(action: str, index: int = 0, item_type: Optional[str] = None) -> dict:
    ...
    if item_type is not None:
        return execute_action(
            "shop_buy", index=index, item_type=item_type, shop_action=normalized
        )
...
def get_exceptions(max_count: int = 20, since_id: int = 0) -> dict:
    ...
    return send_request(
        "get_exceptions", {"max_count": max_count, "since_id": since_id}
    )
...
def focus_game_window() -> dict:
    ...
    if sys.platform != "win32":
        return {
            "success": False,
            "error": "focus_game_window only supported on Windows",
        }
    try:
        import ctypes
...
def navigate_to_combat(
    ...
    while time.monotonic() < deadline:
        ...
        screen = (
            result.get("screen", "UNKNOWN").upper()
    ...
    while time.monotonic() < deadline:
        ...
        if screen == prev_screen:
            ...
            if stuck_count >= 5:
                ...
                state["stuck"] = True
                state["error"] = (
                    f"Stuck on {screen} after {len(steps)} steps — needs manual interaction"
                )
                return state
        else:
            stuck_count = 0
