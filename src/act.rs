...
pub(crate) fn draw_monster_sequence(
    ...
    let mut result = crate::encounters::draw_names(weak_pool, weak_slots, &mut rng);
    result.extend(crate::encounters::draw_names(normal_pool, normal_slots, &mut rng));
    result
}
...
mod tests {
    ...
    fn weak_pool_draw_visits_every_monster_it_should_contain_and_no_others() {
        ...
        let sequence =
            crate::encounters::draw_names(OVERGROWTH_WEAK_POOL, OVERGROWTH_WEAK_POOL.len() * 3, &mut rng);
