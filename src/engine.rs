...
impl Status {
    ...
    pub(crate) fn from_name_and_amount(name: &str, amount: i32) -> Vec<Status> {
        match name {
            "Vulnerable" | "VULNERABLE" => vec![Status::Vulnerable; amount.max(0) as usize],
            "Weak" | "WEAK" => vec![Status::Weak; amount.max(0) as usize],
            ...
            "Slippery" | "SLIPPERY" => vec![Status::Slippery(amount)],
            "Strength" | "STRENGTH" => vec![Status::Strength(amount)],
            ...
            "Constrict" | "CONSTRICT" => vec![Status::Constrict(amount)],
            "Artifact" | "ARTIFACT" => vec![Status::Artifact(amount)],
            "Frail" | "FRAIL" => vec![Status::Frail(amount)],
            "Minion" => vec![Status::Minion {
                leader: "Kin Priest".to_string(),
            }],
            "Stun" | "STUN" => vec![Status::Stun; amount.max(0) as usize],
            "Infested" => vec![Status::Infested {
                minion_name: "Wriggler".to_string(),
                minion_hp: 21,
                count: amount,
            }],
            "Tangled" | "TANGLED" => vec![Status::Tangled(amount)],
            "Slow" | "SLOW" => vec![Status::Slow(amount)],
            "Plow" => vec![Status::Plow(amount)],
            "Ringing" | "RINGING" => vec![Status::Ringing; amount.max(0) as usize],
            "NoDraw" => vec![Status::NoDraw; amount.max(0) as usize],
            "Plating" | "PLATING" => vec![Status::Plating(amount)],
            "Vicious" | "VICIOUS" => vec![Status::Vicious(amount)],
