...
def test_inklet_windup_punch_deals_2_damage_three_times():
    ...
    state = _inklet()
    after_jab = apply(state, EndTurnAction())
    assert after_jab.monsters[0].intent == "Windup Punch"

    after_windup = apply(after_jab, EndTurnAction())
...
def test_inklet_piercing_gaze_deals_10_damage():
    ...
    state = _inklet()
    after_jab_1 = apply(state, EndTurnAction())
    after_windup_1 = apply(after_jab_1, EndTurnAction())
    after_jab_2 = apply(after_windup_1, EndTurnAction())
    after_windup_2 = apply(after_jab_2, EndTurnAction())
    after_jab_3 = apply(after_windup_2, EndTurnAction())
    assert after_jab_3.monsters[0].intent == "Piercing Gaze"

    after_gaze = apply(after_jab_3, EndTurnAction())
...
def test_slippery_caps_hp_loss_to_1_then_is_consumed():
    ...
    after_first = apply(apply(state, PlayCardAction("Strike")), SelectTargetAction(0))
    ...
    after_second = apply(
        apply(after_first, PlayCardAction("Strike")), SelectTargetAction(0)
    )
...
def test_slippery_with_multiple_stacks_caps_each_hit_until_exhausted():
    ...
    after_third = apply(
        apply(after_second, PlayCardAction("Strike")), SelectTargetAction(0)
    )
