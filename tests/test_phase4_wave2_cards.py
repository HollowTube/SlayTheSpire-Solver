...
def test_dark_embrace_draws_a_card_when_a_card_is_exhausted():
    ...
    after_dark_embrace = apply(state, "PlayCard:DarkEmbrace")
    hand_size_before = len(after_dark_embrace.hand)

    after_impervious = apply(after_dark_embrace, "PlayCard:Impervious")
...
def test_feel_no_pain_gains_3_block_when_a_card_is_exhausted():
    ...
    after_feel_no_pain = apply(state, "PlayCard:FeelNoPain")

    after_impervious = apply(after_feel_no_pain, "PlayCard:Impervious")
...
def test_barricade_keeps_block_through_turn_start():
    ...
    after_barricade = apply(state, "PlayCard:Barricade")
    after_block = apply(after_barricade, "PlayCard:ShrugItOff")
    assert after_block.player_block == 8

    after_turn = apply(after_block, "EndTurn")
...
def test_without_barricade_block_clears_at_turn_start():
    ...
    after_block = apply(state, "PlayCard:ShrugItOff")
    assert after_block.player_block == 8
