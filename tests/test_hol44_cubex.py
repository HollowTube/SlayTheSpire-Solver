...
def test_artifact_blocks_one_debuff_application():
    ...
    after = apply(apply(state, "PlayCard:Bash"), "SelectTarget:Monster:0")
...
def test_artifact_blocks_all_debuffs_until_exhausted():
    ...
    a1 = apply(apply(state, "PlayCard:Bash"), "SelectTarget:Monster:0")
    ...
    a2 = apply(apply(a1, "PlayCard:Bash"), "SelectTarget:Monster:0")
...
def test_artifact_blocks_debuff_applied_to_player():
    ...
    a1 = apply(state, "EndTurn")  # Bellow → select next
    assert a1.monsters[0].intent == "Skull Bash"
    a2 = apply(a1, "EndTurn")  # Skull Bash: 6 dmg + 2 Vulnerable
...
def test_repeater_blast_deals_7_damage_plus_strength_and_grants_2_more():
    ...
    state = _cubex()
    a1 = apply(state, "EndTurn")  # Charge Up → 2 Str, selects Repeater
...
def test_two_repeater_blasts_then_expel_blast():
    ...
    state = _cubex()
    a1 = apply(state, "EndTurn")  # Charge Up → Repeater selected

    a2 = apply(a1, "EndTurn")  # 1st Repeater → Repeater selected again
    assert a2.monsters[0].intent == "Repeater Blast"

    a3 = apply(a2, "EndTurn")  # 2nd Repeater → Expel selected
...
def test_expel_blast_deals_5_damage_twice_plus_strength():
    ...
    state = _cubex()
    a1 = apply(state, "EndTurn")  # Charge Up → 2 Str
    a2 = apply(a1, "EndTurn")  # 1st Repeater → 4 Str
    # After 2nd Repeater: 2 + 2 + 2 = 6 Str
    a3 = apply(a2, "EndTurn")  # 2nd Repeater → selects Expel, Str=6
    assert a3.monsters[0].intent == "Expel Blast"
...
def test_cubex_cycle_back_to_repeater_after_expel():
    ...
    state = _cubex()
    a1 = apply(state, "EndTurn")  # Charge Up
    a2 = apply(a1, "EndTurn")  # 1st Repeater
    a3 = apply(a2, "EndTurn")  # 2nd Repeater
    a4 = apply(a3, "EndTurn")  # Expel
