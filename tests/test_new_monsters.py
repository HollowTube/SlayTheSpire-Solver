...
def test_nibbit_cycles_butt_hesitant_slice_hiss_and_repeats():
    ...
    after_butt = apply(state, EndTurnAction())
    assert after_butt.monsters[0].intent == "Hesitant Slice"

    after_hesitant = apply(after_butt, EndTurnAction())
    assert after_hesitant.monsters[0].intent == "Hiss"

    after_hiss = apply(after_hesitant, EndTurnAction())
...
def test_nibbit_hiss_strength_amplifies_subsequent_butt():
    ...
    state = _nibbit()
    after_butt = apply(state, EndTurnAction())  # Butt (12 dmg)
    after_hesitant = apply(after_butt, EndTurnAction())  # Hesitant Slice
    after_hiss = apply(after_hesitant, EndTurnAction())  # Hiss (+2 Str)
    assert after_hiss.monsters[0].intent == "Butt"

    hp_before = after_hiss.player_hp
    after_second_butt = apply(after_hiss, EndTurnAction())
...
def test_fwc_alternates_acid_goop_and_inhale():
    ...
    after_goop = apply(state, EndTurnAction())
    assert after_goop.monsters[0].intent == "Inhale"

    after_inhale = apply(after_goop, EndTurnAction())
    assert after_inhale.monsters[0].intent == "Acid Goop"

    after_goop2 = apply(after_inhale, EndTurnAction())
...
def test_fwc_acid_goop_scales_with_accumulated_strength():
    ...
    state = _fwc()
    after_goop = apply(state, EndTurnAction())  # Acid Goop (4 dmg)
    after_inhale = apply(after_goop, EndTurnAction())  # Inhale (+7 Str)
    assert after_inhale.monsters[0].intent == "Acid Goop"
...
def test_byrdonis_alternates_swoop_and_peck():
    ...
    after_swoop = apply(state, EndTurnAction())
    assert after_swoop.monsters[0].intent == "Peck"

    after_peck = apply(after_swoop, EndTurnAction())
...
def test_byrdonis_strength_amplifies_subsequent_moves():
    ...
    state = _byrdonis()
    after_swoop = apply(state, EndTurnAction())  # Swoop (17 dmg, +1 Str)
    assert after_swoop.monsters[0].intent == "Peck"
    assert after_swoop.monsters[0].strength == 1
